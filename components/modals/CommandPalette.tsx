...
export default function CommandPalette({ open, onClose }: CommandPaletteProps) {
  ...
  function handleSelect(idx: number) {
    if (mode === 'ai') {
      ...
      if (!w) return
      setActiveSidebarTab(w.id === 'prd' || w.id === 'stories' || w.id === 'general' ? 'chat' :
                          w.id === 'roadmap' ? 'roadmap' :
                          w.id === 'prioritization' ? 'prioritize' :
                          w.id === 'research' ? 'research' : 'data')
      toast.info(`Opened ${w.label}`)
      onClose()
    } else {
